...
export const Dropdown = ({ options, id, children, title, position }) => {
  ...
  const handleClickOutside = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target instanceof HTMLElement && !myRef?.current?.contains(e.target)) {
  ...
  return (
    <div ref={myRef} onClick={handleClickInside}>
      <div className={Style['wrapper']} onClick={() => setOpenDropDown(!isOpenDropDown)}>
        {children}
      </div>
            ...
            {options.map(({ label, type, action, logo, icon }) => {
              return (
                <li
                  ...
                  key={label}
                  className={type ? `${Style[type]} ${Style.option}` : Style.option}
                >
                  {logo && <img src={logo} alt={`logo ${label}`} />}
                  {icon && <Icon width="16" height="16" name={icon} />}
                  <p>{label}</p>
                </li>
              );
            })}
